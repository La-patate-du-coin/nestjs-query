...
`@la-patate-du-coin/nestjs-query-core` also provides a number of base `QueryService`s that can be used to create custom query services.
...
The `nestjs-query` and `typeorm` packages provide a module that will add providers
...
:::note
This section only applies when using typeorm with relations
:::
...
:::note
This section only applies when you combine your DTO and entity and are using typeorm
:::
